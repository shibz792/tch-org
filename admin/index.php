...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!empty($user['force_password_change']) && $action !== 'change_password' && $action !== 'logout') {
            ...
            archive_person((int) $_POST['id']);
            $notice = 'Person archived.';
        } elseif ($action === 'delete_person') {
            delete_person((int) $_POST['id']);
            $notice = 'Person deleted.';
            ...
            if($managerId){$stmt=db()->prepare("SELECT * FROM personnel WHERE id=? AND status='active'");$stmt->execute([$managerId]);if(!$stmt->fetch())throw new InvalidArgumentException('Manager must be active.');}
            ...
            else { db()->prepare('INSERT INTO departments(name,color,description,display_order) VALUES(?,?,?,?)')->execute([$name,$color,trim((string)$_POST['description']),(int)$_POST['display_order']]); $id=db_last_insert_id('departments'); }
            audit('save', 'department', $id, null, ['name'=>$name,'color'=>$color]); $notice='Department saved.';
        } elseif ($action === 'save_warehouse') {
            $id = !empty($_POST['id']) ? (int) $_POST['id'] : null;
            $code = strtoupper(trim((string) ($_POST['code'] ?? '')));
            $name = trim((string) ($_POST['name'] ?? ''));
            $status = ($_POST['status'] ?? 'active') === 'inactive' ? 'inactive' : 'active';
            if (!$code || !preg_match('/^[A-Z0-9][A-Z0-9_-]{1,15}$/', $code)) throw new InvalidArgumentException('Warehouse code must contain 2–16 letters, numbers, hyphens, or underscores.');
            if (!$name) throw new InvalidArgumentException('Warehouse name is required.');
            $data = [$code,$name,trim((string)($_POST['location']??'')),trim((string)($_POST['description']??'')),$status,max(0,(int)($_POST['display_order']??0))];
            if ($id) {
                $stmt=db()->prepare('SELECT * FROM warehouses WHERE id=?');$stmt->execute([$id]);$before=$stmt->fetch();if(!$before)throw new InvalidArgumentException('Warehouse not found.');
                db()->prepare('UPDATE warehouses SET code=?,name=?,location=?,description=?,status=?,display_order=?,updated_at=CURRENT_TIMESTAMP WHERE id=?')->execute([...$data,$id]);
            } else {
                db()->prepare('INSERT INTO warehouses(code,name,location,description,status,display_order) VALUES(?,?,?,?,?,?)')->execute($data);$id=db_last_insert_id('warehouses');$before=null;
            }
            audit('save','warehouse',$id,$before,['code'=>$code,'name'=>$name,'status'=>$status]);$notice='Warehouse saved.';
            ...
            $id=db_last_insert_id('users');audit('create','user',$id,null,['name'=>$name,'email'=>$email]);$notice='Administrator created.';
...
if (($_GET['export'] ?? '') === 'csv') {
    ...
    fputcsv($out,['id','name','title','department_id','warehouse_id','location','email','phone','bio','status','manager_id','display_order','is_cherry_global','photo_path']);
    foreach(personnel(true) as $p)fputcsv($out,array_map(fn($k)=>$p[$k]??'', ['id','name','title','department_id','warehouse_id','location','email','phone','bio','status','manager_id','display_order','is_cherry_global','photo_path']));
    audit('export','personnel',null,null,null);exit;
}
$people=personnel(true);$departments=departments();$warehouses=warehouses();$config=settings();$hierarchyIssues=hierarchy_issues();$peopleById=[];
...
<?php foreach(['dashboard'=>'Overview','people'=>'People','hierarchy'=>'Hierarchy','departments'=>'Departments','warehouses'=>'Warehouses','import'=>'Import & backup','users'=>'Administrators','audit'=>'Audit history','settings'=>'Settings'] as $key=>$label): ?><a class="<?= $page===$key?'active':'' ?>" href="?page=<?= $key ?>"><?= e($label) ?></a><?php endforeach; ?></nav>
...
<?php if($hierarchyIssues):?><div class="alert error"><strong>Hierarchy issues need attention:</strong><ul><?php foreach($hierarchyIssues as $issue):?><li><?= e($issue) ?></li><?php endforeach;?></ul></div><?php endif;?>
<?php if($page==='dashboard'):?><section class="metrics"><article><span>Active people</span><strong><?= $active ?></strong></article><article><span>Departments</span><strong><?= count($departments) ?></strong></article><article><span>Warehouses</span><strong><?= count(array_filter($warehouses,fn($w)=>$w['status']==='active')) ?></strong></article><article><span>Archived</span><strong><?= $archived ?></strong></article></section><section class="panel"><h2>Hierarchy health</h2><p>The canonical organization structure is stored in SQLite with validated reporting lines, warehouse assignments, transaction-safe writes, and audit history.</p><a class="primary inline" href="?page=people">Manage people</a></section>
<?php elseif($page==='people'):?><section class="split"><div class="panel"><div class="panel-heading"><div><h2>Personnel directory</h2><p><?= count($people) ?> records</p></div><input id="tableSearch" type="search" placeholder="Search people…"></div><div class="table-wrap"><table><thead><tr><th>Person</th><th>Department</th><th>Warehouse</th><th>Manager</th><th>Status</th><th></th></tr></thead><tbody>
<?php foreach($people as $p):?><tr data-search="<?= e(strtolower($p['name'].' '.$p['title'].' '.$p['department'].' '.$p['warehouse_code'].' '.$p['warehouse_name'])) ?>"><td><strong><?= e($p['name']) ?></strong><small><?= e($p['title']) ?></small></td><td><?= e($p['department']?:'—') ?></td><td><?= e($p['warehouse_code']?:'—') ?></td><td><?= e($peopleById[$p['manager_id']]['name']??'Top level') ?></td><td><span class="status <?= e($p['status']) ?>"><?= e($p['status']) ?></span></td><td><a href="?page=people&edit=<?= $p['id'] ?>">Edit</a></td></tr><?php endforeach;?></tbody></table></div></div>
<?php $edit=null;if(isset($_GET['edit']))foreach($people as $p)if($p['id']==(int)$_GET['edit'])$edit=$p;?><div class="panel sticky"><h2><?= $edit?'Edit person':'Add person' ?></h2><form method="post" enctype="multipart/form-data"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="save_person"><input type="hidden" name="id" value="<?= e($edit['id']??'') ?>"><label>Name<input name="name" required value="<?= e($edit['name']??'') ?>"></label><label>Title<input name="title" value="<?= e($edit['title']??'') ?>"></label><label>Department<select name="department_id"><option value="">None</option><?php foreach($departments as $d):?><option value="<?= $d['id'] ?>" <?= ($edit['department_id']??null)==$d['id']?'selected':'' ?>><?= e($d['name']) ?></option><?php endforeach;?></select></label><label>Primary warehouse<select name="warehouse_id"><option value="">Not assigned</option><?php foreach($warehouses as $w):?><option value="<?= $w['id'] ?>" <?= ($edit['warehouse_id']??null)==$w['id']?'selected':'' ?>><?= e($w['code'].' · '.$w['name'].($w['status']==='inactive'?' (inactive)':'')) ?></option><?php endforeach;?></select></label><label>Reports to<select name="manager_id"><option value="">Top level</option><?php foreach($people as $m):if(($edit['id']??null)==$m['id']||$m['status']!=='active')continue;?><option value="<?= $m['id'] ?>" <?= ($edit['manager_id']??null)==$m['id']?'selected':'' ?>><?= e($m['name']) ?></option><?php endforeach;?></select></label><div class="form-grid"><label>Location<input name="location" value="<?= e($edit['location']??'') ?>"></label><label>Status<select name="status"><option value="active">Active</option><option value="archived" <?= ($edit['status']??'')==='archived'?'selected':'' ?>>Archived</option></select></label></div><div class="form-grid"><label>Email<input type="email" name="email" value="<?= e($edit['email']??'') ?>"></label><label>Phone<input name="phone" value="<?= e($edit['phone']??'') ?>"></label></div><label>Bio<textarea name="bio"><?= e($edit['bio']??'') ?></textarea></label><label>Photo<input type="file" name="photo" accept="image/jpeg,image/png,image/webp"></label><label class="check"><input type="checkbox" name="is_cherry_global" value="1" <?= !empty($edit['is_cherry_global'])?'checked':'' ?>> Cherry Global employee</label><button class="primary">Save person</button></form><?php if($edit&&$edit['status']==='active'):?><form method="post" class="danger-zone"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="archive_person"><input type="hidden" name="id" value="<?= $edit['id'] ?>"><button class="danger">Archive person</button></form><?php endif;?><?php if($edit):?><form method="post" class="danger-zone" onsubmit="return confirm('Delete this person permanently? This cannot be undone.');"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="delete_person"><input type="hidden" name="id" value="<?= $edit['id'] ?>"><button class="danger">Delete person</button></form><?php endif;?></div></section>
<?php elseif($page==='hierarchy'):?><section class="panel"><h2>Visual reporting structure</h2><p>Move a person under a new manager. Circular relationships are rejected before saving, and archived people cannot be managers.</p><div class="hierarchy-admin"><?php $renderTree=function($nodes)use(&$renderTree,$people){echo'<ul>';foreach($nodes as $node){echo'<li><article><strong>'.e($node['name']).'</strong><small>'.e($node['title']).'</small><form method="post"><input type="hidden" name="csrf_token" value="'.e(csrf_token()).'"><input type="hidden" name="action" value="move_person"><input type="hidden" name="id" value="'.(int)$node['id'].'"><select name="manager_id"><option value="">Top level</option>';foreach($people as $m){if($m['id']==$node['id']||$m['status']!=='active')continue;echo'<option value="'.(int)$m['id'].'">'.e($m['name']).'</option>';}echo'</select><button class="secondary">Move under</button></form></article>';if($node['children'])$renderTree($node['children']);echo'</li>';}echo'</ul>';};$renderTree(hierarchy_data());?></div></section>
...
<?php elseif($page==='warehouses'):?><?php $warehouseEdit=null;if(isset($_GET['edit']))foreach($warehouses as $w)if($w['id']==(int)$_GET['edit'])$warehouseEdit=$w;?><section class="split"><div class="panel"><h2>Warehouses</h2><p>Assign people to a primary warehouse and add future sites without changing the organization structure.</p><div class="table-wrap"><table><thead><tr><th>Code</th><th>Warehouse</th><th>Location</th><th>People</th><th>Status</th><th></th></tr></thead><tbody><?php foreach($warehouses as $w):?><tr><td><strong><?= e($w['code']) ?></strong></td><td><?= e($w['name']) ?></td><td><?= e($w['location']?:'—') ?></td><td><?= (int)$w['personnel_count'] ?></td><td><span class="status <?= e($w['status']==='active'?'active':'archived') ?>"><?= e($w['status']) ?></span></td><td><a href="?page=warehouses&edit=<?= $w['id'] ?>">Edit</a></td></tr><?php endforeach;?></tbody></table></div></div><div class="panel sticky"><h2><?= $warehouseEdit?'Edit warehouse':'Add warehouse' ?></h2><form method="post"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="save_warehouse"><input type="hidden" name="id" value="<?= e($warehouseEdit['id']??'') ?>"><label>Code<input name="code" maxlength="16" required placeholder="TCH3" value="<?= e($warehouseEdit['code']??'') ?>"></label><label>Name<input name="name" required placeholder="TCH Warehouse 3" value="<?= e($warehouseEdit['name']??'') ?>"></label><label>Location<input name="location" value="<?= e($warehouseEdit['location']??'') ?>"></label><label>Description<textarea name="description"><?= e($warehouseEdit['description']??'') ?></textarea></label><div class="form-grid"><label>Status<select name="status"><option value="active">Active</option><option value="inactive" <?= ($warehouseEdit['status']??'')==='inactive'?'selected':'' ?>>Inactive</option></select></label><label>Display order<input type="number" min="0" name="display_order" value="<?= e($warehouseEdit['display_order']??'0') ?>"></label></div><button class="primary">Save warehouse</button></form></div></section>
<?php elseif($page==='import'):?><section class="split"><div class="panel"><h2>CSV import</h2><p>Required columns: <code>name</code>, <code>title</code>, <code>manager_id</code>. Uploads are validated before a transaction-based commit.</p><form method="post" enctype="multipart/form-data"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="preview_csv"><label>CSV file<input type="file" name="csv" accept=".csv,text/csv" required></label><button class="primary">Validate and preview</button></form><?php if(!empty($_SESSION['csv_preview'])):?><div class="csv-preview"><h3>Preview: <?= count($_SESSION['csv_preview']) ?> rows</h3><div class="table-wrap"><table><thead><tr><th>Name</th><th>Title</th><th>Manager ID</th></tr></thead><tbody><?php foreach(array_slice($_SESSION['csv_preview'],0,10) as $row):?><tr><td><?= e($row['name']) ?></td><td><?= e($row['title']) ?></td><td><?= e($row['manager_id']) ?></td></tr><?php endforeach;?></tbody></table></div><form method="post"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="import_confirm"><button class="primary">Import validated rows</button></form></div><?php endif;?></div><div class="panel"><h2>Export and backups</h2><p>Download portable data or create a consistent full database snapshot.</p><a class="primary inline" href="?page=import&export=csv">Export CSV</a><form method="post"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="backup"><button class="secondary">Download backup</button></form></div></section>
